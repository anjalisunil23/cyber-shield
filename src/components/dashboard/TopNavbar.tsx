  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border bg-surface/85 px-4 py-3 backdrop-blur-xl sm:px-6">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <div className="ml-1 flex items-center gap-2 rounded-xl border border-border bg-card px-2 py-1.5 sm:px-3">
          ...
          <div className="hidden leading-tight sm:block">
            ...
            <p className="text-[10px] text-muted-foreground">{me.data?.role || "…"}</p>
          </div>
        </div>
      </div>
    </header>
...
function SearchDropdown({
  ...
  return (
    <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 max-h-80 overflow-y-auto rounded-2xl border border-border bg-card p-3 shadow-2xl">
      {loading && <p className="px-2 py-3 text-xs text-muted-foreground">Searching…</p>}
      {!loading && result && (
        <div className="space-y-3 text-sm">
          <Group title="Cases">
            {result.cases.map((c) => (
              <Link
                ...
                params={{ caseId: c.id }}
                onClick={onClose}
                className="block rounded-lg px-2 py-1.5 text-foreground hover:bg-muted"
              >
          ...
          <Group title="Investigators">
            {result.investigators.map((u) => (
              <div key={u.id} className="rounded-lg px-2 py-1.5 text-muted-foreground">
                {u.full_name} · {u.email}
              </div>
            ))}
